...
if (!message.guild) {
    ...
    if (message.mentions.users.size < 1) return message.channel.send(new Discord.RichEmbed().setColor('RANDOM').setTitle('Bulanık;').setDescription(message.author.tag + ', kullanım: ??bulanık <@kullanıcı>.').setFooter('TheRenk', client.user.avatarURL).setTimestamp());
